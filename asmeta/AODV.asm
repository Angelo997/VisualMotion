...
definitions:
	...
	rule r_ObserverProgram = 
		seq								
			//locationsBlock
			forall $_a in Agent do
				par
					curSeqNum($_a):=0
					localReqCount($_a):=0
					receivedReq($_a):=[]
				forall $_d in Agent do
					waitingForRouteTo($_a,$_d):=false
				endpar
			par
				isLinked(host1,host2):=false
				isLinked(host1,host3):=false
				isLinked(host1,host4):=false
				isLinked(host1,host5):=false
				isLinked(host2,host1):=false
				isLinked(host2,host3):=true
				isLinked(host2,host4):=true
				isLinked(host2,host5):=true
				isLinked(host3,host1):=false
				isLinked(host3,host2):=true
				isLinked(host3,host4):=false
				isLinked(host3,host5):=false
				isLinked(host4,host1):=false
				isLinked(host4,host2):=true
				isLinked(host4,host3):=false
				isLinked(host4,host5):=true
				isLinked(host5,host1):=false
				isLinked(host5,host2):=true
				isLinked(host5,host3):=false
