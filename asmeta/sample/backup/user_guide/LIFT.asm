...
definitions:
	...
	main rule r_Main =
		...
		seq
			...
			par
				r_Fsm[$l, HALTING, attracted(direction($l), $l),<<r_Depart(Lift)>>, MOVING]
				r_Fsm[$l, MOVING, canContinue(direction($l),$l),<<r_Continue(Lift)>>, MOVING]
				r_Fsm[$l, MOVING, not canContinue(direction($l),$l),<<r_Stop(Lift)>>, HALTING]
				r_Fsm[$l, HALTING, not attracted(direction($l), $l) and attracted(opposite(direction($l)), $l),<<r_Change(Lift)>>, HALTING]
			endpar
		endseq
default init s0:
