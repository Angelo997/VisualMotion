...
signature:
	...
	dynamic controlled completedRun: Boolean
	...
	dynamic controlled waitingTime: Integer
	...
	derived verifyPendingConnection: Boolean
...
definitions:
	...
	function verifyPendingConnection = 
		(exist $init in Agent, $dest in Agent with ($init != $dest and waitingForRouteTo($init,$dest) = true))
	...
	/*rule r_ObserverProgram = 			
		if (completedRun = false) then
			if (verifyPendingConnection = false) then
				completedRun := true
			else
				if (waitingTime > 0) then
					...
					par							
						forall $a in Agent do	
							forall $dest in Agent with($dest != $a and waitingForRouteTo($a,$dest) = true) do
								waitingForRouteTo($a,$dest) := false
						completedRun := true
					endpar
				endif
	...
	main rule r_Main = 
		seq 
			...
			while (waitingTime > 0 and verifyPendingConnection) do
				seq
					forall $a in Agent do
						program($a) 
					r_MobilityModel[] 
