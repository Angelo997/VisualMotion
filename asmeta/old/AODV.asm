...
*
* aggiunte metriche
* aggiunta struttura PreviouslyReceivedRREQs
* modificato alreadyReceived
*/
...
signature:
	...
	*/
	...
	dynamic controlled previouslyReceivedRREQs: Agent -> Seq(Message)
...
definitions:
	...
	function alreadyReceived($m in Message, $a in Agent) =
		(exist $q in asSet(previouslyReceivedRREQs($a)) with (initiator($m) = initiator($q) and broadcastid($m) = broadcastid($q))) 
	...
	rule r_enqueue($m in Message) = 
		previouslyReceivedRREQs(self) := append(previouslyReceivedRREQs(self),$m) 
