...
signature:
	...
	derived routingTableSize: Integer
	// Routing Table Updates
	dynamic controlled rtu_rreq: Agent -> Integer
	dynamic controlled rtu_rrep: Agent -> Integer	
	// Broadcast Activations
	dynamic controlled ba: Agent -> Integer		
	// Rate of Success
	dynamic controlled rs: Agent -> Integer	
	// Control Overhead
	derived controlOverhead: Integer
...
definitions:
	...
	*/
	...
	function routingTableSize = size(RoutingTable)
	
	/*
	* 	controlOverhead
	*/
	function controlOverhead = size(Message)
	...
	rule r_ObserverProgram = 
		seq
			// metrics init
			rts := routingTableSize	
			if(isInitialized = undef) then	
				co := 0
			endif
			if(isInitialized = true) then	
				co := controlOverhead - co
			endif
			forall $ag in Agent do
				par
					rtu_rreq($ag) := 0
					rtu_rrep($ag) := 0
					ba($ag) := 0
