...
signature:
	...
	dynamic controlled messageCHL: Message -> Prod(Agent, Agent, Integer, Prod(Integer,Agent))
	...
	derived localId: Prod(Integer, Agent, Integer) -> Integer
	derived origin: Prod(Integer, Agent, Integer) -> Agent
	derived nonce: Prod(Integer, Agent, Integer) -> Integer
	...
	derived globalId: Prod(Integer,Agent,Integer) -> Prod(Integer, Agent)
...
definitions:
	...
	function origin($m in Message) =
		switch messageType($m)
			...
			case CHL: second(fourth(messageCHL($m)))
			case RES: second(fourth(messageRES($m)))
		endswitch
	
	function origin($localId in Integer, $origin in Agent, $nonce in Integer) =
		$origin
	...
	function localId($m in Message) =
		switch messageType($m)
			...
			case CHL: first(fourth(messageCHL($m)))
			case RES: first(fourth(messageRES($m)))
		endswitch	
		
	function localId($localId in Integer, $origin in Agent, $nonce in Integer) =
		$localId 
	...
	function nonce($localId in Integer, $origin in Agent, $nonce in Integer) =
		$nonce 
	...
	function alreadyReceivedBefore($m in Message) =
		(exist $r in asSet(receivedReq(self)) with (globalId($r) = globalId($m)))
		
	function globalId($m in Message) =
		(localId($m), origin($m))	
		
	function globalId($localId in Integer, $origin in Agent, $nonce in Integer) =
		($localId, $origin)	 
	...
	function trustedRES($m in Message) =
		(exist $r in asSet(receivedReq(self)) with (globalId($r)=globalId($m) and nonce($r) = nonce($m) - 1))
	...
	rule r_Buffer($m in Message) =
		choose $nonce in asSet([1..100]) with true do skip
			//receivedReq(self) := append(receivedReq(self), (localId($m),origin($m),$nonce))
	...
	rule r_GenerateChl($m in Message) = 
		choose $r in asSet(receivedReq(self)) with (globalId($r)=globalId($m)) do
