...
signature:
	...
	dynamic controlled errors: Prod(Agent, Message, Integer) -> Agent
	...
	dynamic controlled replies: Prod(Agent, Message, Integer) -> Agent		
	//dynamic controlled requests: Agent -> Seq(Message)	
	dynamic controlled requests: Prod(Agent, Message, Integer) -> Agent		
...
definitions:
	...
	rule r_Send($m in Message, $n in Agent) = 		
		switch messageType($m) 
			case RREQ: 
				par
					requests(self, $m, currTimeNanosecs) := $n
					isConsumed($m) := false
				endpar
			case RREP: 
				par
					replies(self, $m, currTimeNanosecs) := $n
					isConsumed($m) := false
				endpar
			case RERR: 
				par
					errors(self, $m, currTimeNanosecs) := $n
					isConsumed($m) := false
				endpar  
		endswitch
	...
	rule r_PropagateRouteErr = 
		choose $rerr in Message with (messageType($rerr) = RERR) do
			par
	...
	rule r_ProcessRouteReq =
		choose $rreq in Message with (messageType($rreq) = RREQ /*and requests($rreq))=self*/ and isConsumed($rreq) = false) do
			par
	...
	rule r_ProcessRouteRep =
		choose $rrep in Message with (messageType($rrep) = RREP) do
			par
