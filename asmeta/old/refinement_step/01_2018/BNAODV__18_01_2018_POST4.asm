...
definitions:
	...
	function localId($m in Message) =
		switch messageType($m)
			...
			case RREP: first(sixth(messageRREP($m)))
	...
	function trustedRES($m in Message) =
		if (nonce(self,globalId($m))=nonce($m) - 1) then
			true
		else
			false
		endif
