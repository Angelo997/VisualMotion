...
signature:
	...
	dynamic controlled fresh: Any -> Boolean
...
definitions:
	...
	rule r_Contract =
		par
			forall $e1 in Malicious do
				fresh($e1) := true
			forall $e2 in MaliciousMessage do
				fresh($e2) := true
			forall $e3 in Message do
				fresh($e3) := true
			forall $e4 in RoutingTable do
				fresh($e4) := true	
			forall $e5 in Success do
				fresh($e5) := true	
			forall $e7 in Trial do
				fresh($e7) := true	
			forall $e8 in Update do
				fresh($e8) := true	
		endpar			
	
	rule r_StartCommunicationWith($dest in Agent) = 
		choose $e in Success with (fresh($e)) do
			fresh($e) := false
		ifnone
			extend Success with $newsuccess do 
				fresh($newsuccess) := false	
	...
	rule r_GenerateRouteErr = 
		forall $e in RoutingTable with (owner($e)=self and active($e) and linkBreak(entryNextHop($e)) and not(isEmpty(precursor($e)))) do 
			seq
				active($e) := false
				choose $el in Update with (fresh($el)) do
					fresh($el) := false
				ifnone
					extend Update with $newupdate do 
						fresh($newupdate) := false	
				entry($e) := (entryDest($e), entrySeqNum($e) + 1, entryHopCount($e), entryNextHop($e))
				choose $elem in Message with (fresh($elem)) do
					seq
						fresh($elem) := false
						messageType($elem) := RERR
						messageRERR($elem) := (entryDest($e),
												entrySeqNum($e),
												self)
					...
					extend Message with $newrerr do 
						seq
							...
							messageRERR($newrerr) := (entryDest($e),
													entrySeqNum($e),
													self)
							forall $ag in asSet(precursor($e)) do
								r_Send[$newrerr, $ag]
							precursor($e) := []
						endseq
			endseq
	...
	rule r_GenerateRouteReq($dest in Agent) = 	
		choose $e in Message with (fresh($e)) do
			seq
				fresh($e) := false
			...
			extend Message with $newrrep do 
				seq
					...
					if (dest($m) = self) then
						...
						seq
							r_EntryFor[dest($m)] 
							let ($ffwdEntry = entryFor(self)) in
								seq
									messageType($newrrep) := RREP
									messageRREP($newrrep) := (origin($m), 
															entryHopCount($ffwdEntry),  
															dest($m), 
															entrySeqNum($ffwdEntry),
															self,
															globalId($m))							
									r_PrecursorInsertion[sender($m), $ffwdEntry] 
								endseq
							endlet	
						endseq					
					endif
		...
		choose $rerr in Message, $t in Time with (errors($rerr,set($t))=self and isConsumed($rerr,set($t))=false) do
			par
				forall $e in RoutingTable with (owner($e)=self and entryDest($e)=dest($rerr) and entryNextHop($e)=sender($rerr)) do
					par
						active($e) := false
						choose $el in Update with (fresh($e)) do
							fresh($el) := false
						ifnone
							extend Update with $newupdate do
								fresh($newupdate) := false
		...
		forall $dest in Agent with ($dest != self) do
			choose $wantsToCommunicateWith in Boolean with true do
				if ($wantsToCommunicateWith) then
					par
						choose $e in Trial with (fresh($e)) do
							fresh($e) := false
						ifnone
							extend Trial with $newtrial do 
								fresh($newtrial) := false
	...
	rule r_ExtendReverseRoute($m in Message) = 
		choose $e in RoutingTable with (fresh($e)) do
			seq
				fresh($e) := false
				owner($e) := self
				entry($e) := (origin($m), undef, undef, undef) 
				precursor($e) := []
				r_UpdateReverseRoute[$e, $m]
			endseq
		ifnone
			extend RoutingTable with $newentry do 
				seq
					fresh($newentry) := false
					owner($newentry) := self
			...
			seq
				...
				switch messageType($m)
					case RREP:
						seq
							owner($e) := self
							entry($e) := (dest($m), undef, undef, undef)
							precursor($e) := [] 
							r_UpdateForwardRoute[$e, $m]
						endseq
					case NACK:
						seq
							owner($e) := self
				...
				if (hasNewForwardRouteInfo($m)) then
					seq
						...
						messageRREP($e) := (origin($m),  
												...
												globalId($m))			
						r_EntryFor[origin($m)]	
						r_Send[$e, entryNextHop(entryFor(self))]
					endseq
				else
					seq
						r_EntryFor[dest($m)] 
			...
			par
				...
				if (waitingTime($rrep) = undef) then
					...
					if (waitingTime($rrep) < 5) then 
						...
						par
							choose $e in Malicious with (fresh($e)) do
								seq
									fresh($e) := false
									interceptedBlackhole($e, self) := sender($rrep)
								endseq
							ifnone
								extend Malicious with $newmalicious do 
									seq
										fresh($newmalicious) := false
	...
	rule r_BlackHoleProgram =
		choose $rreq in Message, $t in Time with (requests($rreq,set($t))=self and isConsumed($rreq,set($t))=false) do
			par
				...
				choose $e in MaliciousMessage with (fresh($e)) do
					fresh($e) := false
				ifnone
					extend MaliciousMessage with $newrrep do 
						fresh($newrrep) := false 				
				r_Consume[$rreq]
			endpar	
		...
		choose $rrep in Message with (replies($rrep)=self and isConsumedRREP($rrep)=false) do
			par
				...
				choose $e in MaliciousMessage with (fresh($e)) do
					fresh($e) := false
				ifnone
					extend MaliciousMessage with $fwdrrep do 
						fresh($fwdrrep) := false
				r_Consume[$rrep]
			endpar
