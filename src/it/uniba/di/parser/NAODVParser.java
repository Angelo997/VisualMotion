 ...
 * @author Marco Pinto
 */
public class NAODVParser extends Utility {
	...
	public static final String INST_NACK = "inst_nack";
				...
				while ((line = br.readLine()) != null) {
					if (finalState) {
						...
						if (line.startsWith("messageType") || line.startsWith("messageRREQ")
								...
								|| line.startsWith("messageNACK") || line.startsWith("isConsumed")) {
							try {
	...
	public static HashMap<String, Integer> parser(String outputFile) throws IOException {
		...
		try (FileReader in = new FileReader(outputFile); BufferedReader br = new BufferedReader(in)) {
			...
			while ((line = br.readLine()) != null) {
				if (finalState) {
					...
					if (line.startsWith("rreq_update") || line.startsWith("rrep_update")
							|| line.startsWith("rerr_update") || line.startsWith("nack_update")) {
			...
			displayInfo("ERROR: Problem reading file (NAODVParser.parser)");
			error(ex);
		}
		return metricsMap;
	}
